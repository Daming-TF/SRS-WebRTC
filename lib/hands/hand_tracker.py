...
from lib.utils.draw import draw_point, draw_rectangle, draw_rotated_rect, draw_text, copy_past_roi, Draw3dLandmarks, \
    draw_gesture
...
class HandTracker(object):
    def __init__(self, frame_size=None, capability=1, threshold=0.5, pipe_mode=0, is_draw3d=False, roi_mode=0):
        if frame_size is None:
            self.priori_box = [(300, 200), (700, 500)]        # get a priori box with kinect or hand detector
        else:
    ...
    def __call__(self, img_bgr):
        ...
        start = time.time()
        pose_preds, handness, righthand_props, roi_boxes, rects, world_landmarks = \
        ...
        for coords, is_hand, righthand_prop, coords_last, hand_box, roi_box, rect, world_landmark in zip(
                pose_preds, handness, righthand_props, self.pts_buffer, self.hand_boxes, roi_boxes, rects,
                world_landmarks):
            ...
            if is_hand > self.landmark_thres:
                ...
                if self.pipe_mode == 0:
                    ...
                    img_show = draw_gesture(
                        img_show, coords, recognize_gesture(self.hand_model.unprojected_world_landmarks))
            else:
