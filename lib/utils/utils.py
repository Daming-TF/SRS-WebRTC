...
def smooth_pts(pts_last, pts, box):
    ...
    """
    x_left, y_top = box[0]
    x_right, y_bottom = box[1]
    thresh = max(x_right-x_left, y_bottom-y_top) / 30
...
def smooth_region(idx_region, pts_last, pts, thresh):
    ...
    ratio = 1. - 1. / (1. + np.exp(-3. / thresh * (dist - thresh)))
    return ratio * pts_last[idx_region, :] + (1. - ratio) * pts[idx_region, :]
...
def get_rotation_matrix(radian):
    matrix = np.array([[math.cos(radian), math.sin(radian), 0],
